...
class Coppock(object):
    def __init__(self, data: DataFrame, params: dict={'wma_pd':10, 'roc_long':14, 'roc_short':11}):
        self.wma_pd = params['wma_pd']
        self.long_pd = params['roc_long']
        self.short_pd = params['roc_short']
        self.past_data = None

        self.values = self.calc_copp(np.array(data))
