...
class Genlabels(object):
    ...
    def __init__(self, data: DataFrame, params: dict={'window': 25, 'polyorder': 3}, graph: bool=False):
        # check for valid parameters
        try:
            if params['window']%2 == 0: raise ValueError('Window length must be an odd positive value')
            if params['polyorder'] >= params['window']: raise ValueError('Polyorder must be smaller than windows length')
        except ValueError as error:
            sys.exit('Error: {0}'.format(error))   

        # load historic data from file
        self.hist = np.array(data)
        self.window = params['window']
        self.polyorder = params['polyorder']
